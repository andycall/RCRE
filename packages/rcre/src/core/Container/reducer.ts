...
export let initialState: IContainerState = {
    [TMP_MODEL]: {}
};

export function initContainerState(model: string, data: object) {
    if (!initialState[model]) {
        initialState[model] = {};
    }

    Object.assign(initialState[model], data);
}
    ...
    switch (actions.type) {
        case INIT_CONTAINER: {
            ...
            if (container.parent) {
                ...
                syncPropsContainerState(state, container.parent, actions.context);
                syncExportContainerState(state, affectNode, container, actions.context);

                affectNode.forEach(node => {
                    syncPropsContainerState(state, node, actions.context);
                });
            }
        ...
        case ASYNC_LOAD_DATA_SUCCESS: {
            ...
            syncExportContainerState(state, affectNode, container, {});
            affectNode.forEach(node => {
                syncPropsContainerState(state, node, payload.context);
            });
        ...
        case SYNC_LOAD_DATA_SUCCESS: {
            ...
            if (container && container.parent) {
                affectNode.push(container.parent);
                syncPropsContainerState(state, container.parent, payload.context);
            }
            syncExportContainerState(state, affectNode, container, {});
            affectNode.forEach(node => {
                syncPropsContainerState(state, node, payload.context);
        ...
        case DELETE_DATA: {
            ...
            if (container && container.options.syncDelete) {
                ...
                state[model] = deleteWith(name, state[model]);
                syncExportContainerState(state, [], container, actions.context);
            }
