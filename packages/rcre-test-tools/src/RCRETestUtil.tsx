...
import {PageConfig, JSONRender, RootState, RCREState} from 'rcre';
import {Store} from 'redux';
import {ExecTaskOptions} from '../../rcre/src/types';
...
export class RCRETestUtil {
    ...
    public getComponentByType(type: string, index: number = 0): ReactWrapper {
        ...
        for (let i = 0; i < container.children().length; i++) {
            this.find(container.childAt(i), elements, (element) => {
                ...
                if (name === 'RCREConnect(' + type + ')') {
                    return true;
                }

                if (name === 'ES' && element.prop('type') === type) {
                    return true;
                }
    ...
    public getComponentByName(name: string, index: number = 0): ReactWrapper {
        ...
        for (let i = 0; i < container.children().length; i++) {
            this.find(container.childAt(i), elements, (element) => {
                ...
                if (element.name() === 'ES' && element.prop('name') === name) {
                    return true;
                }
    ...
    /**
     * 触发组件所附属Container组件的一个Task or TaskGroup
     * @param component
     * @param task
     * @param args
     * @param options
     */
    public async execTask(component: ReactWrapper, task: string, args: any, options: ExecTaskOptions = {}) {
        let instance: any = component.instance();
        await instance.TEST_execTask(task, args, options);
        this.wrapper.update();
    }
